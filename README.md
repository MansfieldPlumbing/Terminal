...
**Frontend:** A React/Vite SPA served from `Assets/wwwroot/` and rendered in an Android `WebView`. All UI is web-native — no Android XML layouts, no Jetpack Compose.

**IPC Bridge:** Two-way communication via `window.AndroidBridge` (JS → C#, exposed via `@JavascriptInterface`) and `window.receivePwshOutput` (C# → JS, via `WebView.EvaluateJavascript` with Base64-encoded UTF-8 payloads). A queuing handshake ensures no output is dropped during engine boot.

**Applets:** Discrete tools (file explorer, media stitcher, text editor) are single-file `.html` applications stored in `public/applets/` and loaded into the terminal shell via `<iframe>` with `postMessage` routing. New tools require no native Android code.
...
cd ReactUI && npm install && npm run build

# 3. Copy the built frontend into the Android asset tree
cp -r ReactUI/dist/* Assets/wwwroot/
...
### Persistent Process via Android Notification
Android's process lifecycle will kill background services under memory pressure. The fix is a foreground `Service` with a persistent notification, which signals to the OS that this process is user-intentional and should survive. This will move the PowerShell Runspace out of the `Activity` and into a bound foreground `Service`, with the WebView binding to it on startup. The terminal survives app backgrounding, screen off, and system pressure.
