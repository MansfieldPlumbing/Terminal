...
# QuickPS bootstrap: prefer side-by-side, then one folder up.
$calculatorHeadless = [bool]$Headless

function Write-DiagnosticPanel {
...
function Wait-FailureAcknowledge {
...
$dpCandidates = @(
    ...
    (Join-Path (Split-Path $PSScriptRoot -Parent) 'QuickPS.ps1')
    (Join-Path (Split-Path (Split-Path $PSScriptRoot -Parent) -Parent) 'QuickPS.ps1')
)
...
if (-not $dpPath) {
    Write-DiagnosticPanel @(
        ' Calculator.ps1 startup failure '
        ' QuickPS.ps1 was not found. '
        ' Checked: '
        "   $($dpCandidates[0]) "
        "   $($dpCandidates[1]) "
    )
    Wait-FailureAcknowledge
    return
}
...
try {
    ...
    if ($null -eq (Get-Command New-Canvas -CommandType Function -ErrorAction SilentlyContinue)) {
        throw 'QuickPS.ps1 loaded, but New-Canvas was not defined.'
    }
}
catch {
...
function Start-Calculator([switch] $Headless) {
    ...
    $gpu = New-Canvas -Width 340 -Height 520 -Title "Calculator" -Borderless -Headless:$Headless
    ...
    try {
        ...
        # surface invalidation or a visual calculator-state change.
        $state.RenderRequested = $false

        $runtime = Get-Variable -Name QuickPSRuntimeState -Scope Global -ValueOnly -ErrorAction SilentlyContinue
        ...
        Write-DiagnosticPanel @(
            ...
            " QuickPS  $dpPath "
            " Runtime     QuickPS $runtimeVersion ($runtimePlatform) "
        ...
        while ($gpu.Alive) {
            ...
            # Render only when QuickPS reports that the surface itself needs
