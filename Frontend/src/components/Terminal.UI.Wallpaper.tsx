...
const fsSource = `
  ...
  void main() {
      ...
      // Soften and blend the clouds into the sky
...
export default function WebGLWallpaper() {
  ...
  useEffect(() => {
    ...
    let lastDrawTime = 0;
    const fpsInterval = 1000 / 24; // Throttle to 24 FPS for battery life

    const render = (time: number) => {
      animationId = requestAnimationFrame(render);
      
      const elapsed = time - lastDrawTime;
      if (elapsed < fpsInterval) return; // Skip frame to throttle
